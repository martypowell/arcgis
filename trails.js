require(["myPlaceholder", "bootstrap", "esri/dijit/Legend", "underscore",
    "doTimeout", "esri/map", "esri/SpatialReference", "esri/geometry/Extent", "esri/tasks/RelationshipQuery", "dojo/promise/all", "esri/InfoTemplate", "jquery", "mustache", "esri/dijit/editing/AttachmentEditor", "esri/geometry/Point", "esri/layers/FeatureLayer", "esri/tasks/query", "esri/TimeExtent",
    "dojo/number", "dojo/date/locale", "dojo/dom", "dojo/on",
    "dojo/_base/array", "dojo/store/Memory",
    "dgrid/OnDemandGrid", "dojo/domReady!"], function (myPlaceholder, Bootstrap, Legend, _, doTimeout, Map, SpatialReference, Extent, RelationshipQuery, all, InfoTemplate, $, Mustache, AttachmentEditor, Point, FeatureLayer, Query, TimeExtent, number, locale, dom, on, arrayUtils, Memory, OnDemandGrid) {
    ...
    map.on("load", function () {
        ...
        parkLayer = new FeatureLayer("http://gis.baltimorecountymd.gov/arcgis/rest/services/Apps/WalkingTrailsExternalViewer/MapServer/0", {
            name: "parks",
        ...
        trailLayer = new FeatureLayer("http://gis.baltimorecountymd.gov/arcgis/rest/services/Apps/WalkingTrailsExternalViewer/MapServer/1", {
            name: "trails",
            mode: esri.layers.FeatureLayer.MODE_SNAPSHOT,
            outFields: ["*"], //Return all fields from the layer
    ...
    GetParksList = function (filter) {
        ...
        var query = new Query();
        query.where = "NAME != 'null' ORDER BY NAME";
    ...
    buildQueryParams = function (input) {
        return "ZIPCODE LIKE '" + input +
            ...
            "%') or UPPER(NAME) LIKE UPPER('%" + input + "%') AND NAME != 'null' ORDER BY NAME";
    },
    ...
    queryRecordsByPage = function (pageNumber, anchor, callback) {
        ...
        if (results) {
            ...
            parkLayer.queryFeatures(query, function (results) {
                storeData("parks", {
                    query: query,
                    data: results
                });
